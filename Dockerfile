...
RUN apk --no-cache --virtual build-deps add   \
    ...
    && install -d /usr/sbin/ \
    && install -s -m755 /usr/local/src/mosquitto-1.6.3/src/mosquitto /usr/sbin/mosquitto \
    && install -s -m755 /usr/local/src/mosquitto-1.6.3/src/mosquitto_passwd /usr/bin/mosquitto_passwd \
    && install -m644 /usr/local/src/mosquitto-1.6.3/mosquitto.conf /mosquitto/config/mosquitto.conf \
    ...
    && git clone --single-branch -b development https://github.com/Vinayakj009/mosquitto-auth-plug.git \
    ...
    && sed -i "s|MOSQUITTO_SRC =|MOSQUITTO_SRC = /usr/local/src/mosquitto-1.6.3|g" config.mk \
    && make clean \
    && make -j "$(nproc)" \
